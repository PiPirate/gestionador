    ...
    <x-modules.shell>
        <div class="flex items-center justify-between mb-4">
            ...
            <form method="GET" action="{{ route('reports.export.transactions') }}" class="flex items-center gap-2">
                ...
                <button
                    class="inline-flex items-center gap-2 bg-blue-700 text-white px-4 py-2 rounded-md text-sm shadow-sm hover:bg-blue-800">
        ...
        <form method="GET" action="{{ route('reports.index') }}"
            class="bg-white border rounded-lg p-4 mb-4 flex flex-wrap items-end gap-3 shadow-sm">
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <x-modules.card title="Transacciones">
                ...
                <p class="text-xs text-gray-600">Volumen:
                    {{ \App\Support\Currency::format($metrics['transactions']['volume_usd'], 'usd') }}</p>
                <p class="text-xs text-green-700">Ganancia:
                    {{ \App\Support\Currency::format($metrics['transactions']['profit_cop'], 'cop') }}</p>
            </x-modules.card>
            ...
            <x-modules.card title="Liquidaciones">
                ...
                <p class="text-xs text-green-700">Pagado:
                    {{ \App\Support\Currency::format($metrics['liquidations']['paid_cop'], 'cop') }}</p>
            </x-modules.card>
            <x-modules.card title="Caja">
                <p class="text-3xl font-bold text-gray-900">
                    {{ \App\Support\Currency::format($metrics['cash']['income_cop'] - $metrics['cash']['expense_cop'], 'cop') }}
                </p>
                <p class="text-xs text-green-700">Ingresos:
                    {{ \App\Support\Currency::format($metrics['cash']['income_cop'], 'cop') }}</p>
                <p class="text-xs text-red-600">Egresos:
                    {{ \App\Support\Currency::format($metrics['cash']['expense_cop'], 'cop') }}</p>
            </x-modules.card>
        </div>
                ...
                <div class="divide-y divide-gray-100">
                    @forelse ($recentTransactions as $tx)
                        <div class="grid grid-cols-5 py-2 text-sm items-center">
                            ...
                            <span
                                class="text-right text-green-700 font-semibold">{{ \App\Support\Currency::format($tx->profit_cop, 'cop') }}</span>
                        </div>
                    @empty
