    ...
    <x-modules.shell>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <x-modules.card title="Transacciones">
                ...
                <p class="text-xs text-gray-600">Volumen: {{ Currency::format($metrics['transactions']['volume_usd'], 'usd') }}</p>
                <p class="text-xs text-green-700">Ganancia: {{ Currency::format($metrics['transactions']['profit_cop'], 'cop') }}</p>
            </x-modules.card>
            <x-modules.card title="Inversiones">
                <p class="text-3xl font-bold text-gray-900">{{ $metrics['investments']['count'] }}</p>
                <p class="text-xs text-gray-600">Capital: {{ Currency::format($metrics['investments']['capital_usd'], 'usd') }}</p>
            ...
            <x-modules.card title="Liquidaciones">
                ...
                <p class="text-xs text-green-700">Pagado: {{ Currency::format($metrics['liquidations']['paid_cop'], 'cop') }}</p>
            </x-modules.card>
            <x-modules.card title="Caja">
                <p class="text-3xl font-bold text-gray-900">{{ Currency::format($metrics['cash']['income_cop'] - $metrics['cash']['expense_cop'], 'cop') }}</p>
                <p class="text-xs text-green-700">Ingresos: {{ Currency::format($metrics['cash']['income_cop'], 'cop') }}</p>
                <p class="text-xs text-red-600">Egresos: {{ Currency::format($metrics['cash']['expense_cop'], 'cop') }}</p>
            </x-modules.card>
        </div>
                ...
                <div class="divide-y divide-gray-100">
                    @forelse ($topInvestors as $investor)
                        <div class="py-3 flex items-center justify-between text-sm">
                            ...
                            <p class="text-gray-900 font-semibold">{{ Currency::format($investor->investments_sum_amount_usd, 'usd') }}</p>
                        </div>
                    @empty
                ...
                <div class="divide-y divide-gray-100">
                    @forelse ($recentTransactions as $tx)
                        <div class="grid grid-cols-5 py-2 text-sm items-center">
                            ...
                            <span class="text-right">{{ Currency::format($tx->amount_usd, 'usd') }}</span>
                            <span class="text-right">{{ Currency::format($tx->amount_cop, 'cop') }}</span>
                            <span class="text-right text-green-700 font-semibold">{{ Currency::format($tx->profit_cop, 'cop') }}</span>
                        </div>
                    @empty
